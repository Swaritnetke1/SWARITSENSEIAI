...
// Uses Google Drive API v3 via gapi loader

const DRIVE_KEY = 'ss_drive_config';
...
export function getDriveConfig(): DriveConfig {
  try {
    ...
    return raw ? JSON.parse(raw) : { connected: false, email: '', fileId: null, lastBackup: null };
  } catch {
    return { connected: false, email: '', fileId: null, lastBackup: null };
  }
}

export function saveDriveConfig(cfg: DriveConfig) {
  localStorage.setItem(DRIVE_KEY, JSON.stringify(cfg));
}

export function disconnectDrive() {
  saveDriveConfig({ connected: false, email: '', fileId: null, lastBackup: null });
  try {
...
export async function backupToDrive(data: object): Promise<void> {
  ...
  try {
    ...
    saveDriveConfig({ ...cfg, fileId, lastBackup: new Date().toISOString() });
  } catch (e) {
