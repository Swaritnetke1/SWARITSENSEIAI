...
export default function AppV2() {
  ...
  async function handleOnboardingComplete() {
    try {
      const fresh = await dbV2.getSettings();
      setSettings(fresh);
    } catch { /* settings already updated in DB by Onboarding */ }
  }
  ...
  if (!isInitialized) {
    return (
      <div className="min-h-screen bg-gradient-to-br from-slate-950 via-purple-950 to-slate-950 flex items-center justify-center dark">
        <motion.div
        ...
        >
          <motion.div
            ...
            className="w-16 h-16 mx-auto border-4 border-purple-500/30 border-t-purple-400 rounded-full"
          />
          <div>
  ...
  if (!settings || !settings.isOnboardingComplete) {
    return <Onboarding onComplete={handleOnboardingComplete} />;
  }

  const sidebarAppName = (() => { try { return localStorage.getItem('ss_app_name') || 'SwaritSensei'; } catch { return 'SwaritSensei'; } })();
  const sidebarTagline = (() => { try { return localStorage.getItem('ss_tagline') || 'Data-Driven Productivity'; } catch { return 'Data-Driven Productivity'; } })();
          ...
          <div className="p-6 border-b border-white/10">
            <div className="flex items-center gap-3">
              ...
              <div>
                ...
                <p className="text-xs text-muted-foreground">{sidebarTagline}</p>
              </div>
            </div>
          </div>
        ...
        <div className="md:hidden flex flex-col bg-slate-900/80 backdrop-blur-xl border-b border-white/10 sticky top-0 z-50">
          ...
          <div className="flex items-center justify-between px-4 py-3">
            ...
            <div className="flex items-center gap-1">
              <Button variant="ghost" size="sm" onClick={() => setShowSupport(true)} className="text-white/50 hover:text-white hover:bg-white/10">
                <Headphones className="w-4 h-4" />
              </Button>
              <Button variant="ghost" size="sm" onClick={handleSettingsClick} className="text-white/70 hover:text-white hover:bg-white/10">
                <SettingsIcon className="w-5 h-5" />
              </Button>
              <Button variant="ghost" size="sm" onClick={handleLogout} className="text-white/40 hover:text-red-400 hover:bg-red-500/5">
